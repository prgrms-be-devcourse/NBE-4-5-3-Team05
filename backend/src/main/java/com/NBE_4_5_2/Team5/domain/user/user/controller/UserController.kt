...
class UserController(
    ...
    @PostMapping("/signup")
    fun createUser(@RequestBody @Valid userForm: SignUpUserForm): RsData<UserDto> {
    ...
    @PostMapping("/login")
    fun loginUser(@RequestBody @Valid userForm: LoginUserForm): RsData<LoginUserDto> {
        val user = userService.loginUser(userForm.username, userForm.password)
    ...
    @PostMapping("/refresh")
    fun refreshAccessToken(@RequestBody @Valid userForm: RefreshUserForm): RsData<String> {
        val newAccessToken = userService.refreshAccessToken(userForm.refreshToken)
    ...
    fun updateMyProfile(@RequestBody @Valid updateRequest: UserUpdateRequest): RsData<UserDto> {
        val userIdentity = userAuthService.userIdentity
        val user = userAuthService.getRealActor(userIdentity)
        val updatedUser = userService.updateMyProfile(user, updateRequest)
    ...
    @PreAuthorize("isAuthenticated()")
    @DeleteMapping("/me")
    fun deleteMyProfile(): RsData<*> {
        val userIdentity = userAuthService.userIdentity
        val user = userAuthService.getRealActor(userIdentity)
        userService.deleteMyProfile(user)
        rq.removeCookie("accessToken")
        rq.removeCookie("refreshToken")
        return RsData("200", "회원 탈퇴 성공", Empty())
    }
    ...
    fun sendAuthenticationCode(@RequestBody userForm: EmailCodeRequest): RsData<Void> {
    ...
    fun verifyAuthenticationCode(@RequestBody userForm: VerifyCodeRequest): RsData<Void> {
        userService.verifyAuthenticationCode(userForm.email, userForm.code)
