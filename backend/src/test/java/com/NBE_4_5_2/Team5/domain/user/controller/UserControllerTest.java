...
class UserControllerTest {
	...
	void createUser7() throws Exception {
		...
		ResultActions resultActions = mvc.perform(post("/api/users/signup")).andDo(print());
	...
	void loginUser6() throws Exception {
		...
		ResultActions resultActions = mvc.perform(post("/api/users/login")).andDo(print());
	...
	void logoutUser1() throws Exception {
		ResultActions resultActions = mvc.perform(
			post("/api/users/logout").header("Authorization", "Bearer " + validToken));
	...
	void logoutUser2() throws Exception {
		ResultActions resultActions = mvc.perform(
			post("/api/users/logout").cookie(new Cookie("accessToken", validAccessToken))
	...
	void me4() throws Exception {
		...
		ResultActions resultActions = mvc.perform(
			get("/api/users/me").cookie(new Cookie("accessToken", validAccessToken))
				.cookie(new Cookie("refreshToken", validRefreshToken))).andDo(print());
	...
	void me5() throws Exception {
		ResultActions resultActions = mvc.perform(
			get("/api/users/me").cookie(new Cookie("accessToken", expiredAccessToken))
				.cookie(new Cookie("refreshToken", validRefreshToken))).andDo(print());
	...
	void me6() throws Exception {
		ResultActions resultActions = mvc.perform(
			get("/api/users/me").cookie(new Cookie("accessToken", validAccessToken))
				.cookie(new Cookie("refreshToken", invalidRefreshToken))).andDo(print());
	...
	void me8() throws Exception {
		ResultActions resultActions = mvc.perform(
			get("/api/users/me").cookie(new Cookie("refreshToken", validRefreshToken))).andDo(print());
	...
	void me9() throws Exception {
		ResultActions resultActions = mvc.perform(get("/api/users/me").header("Authorization", "Bearer " + validToken))
			.andDo(print());
	...
	void me10() throws Exception {
		ResultActions resultActions = mvc.perform(
				get("/api/users/me").header("Authorization", "Bearer " + validRefreshToken + " " + expiredAccessToken))
			.andDo(print());
	...
	void me11() throws Exception {
		ResultActions resultActions = mvc.perform(
				get("/api/users/me").header("Authorization", "Bearer " + invalidRefreshToken + " " + validAccessToken))
			.andDo(print());
	...
	void me12() throws Exception {
		ResultActions resultActions = mvc.perform(
				get("/api/users/me").header("Authorization", "Bearer " + invalidRefreshToken + " " + expiredAccessToken))
			.andDo(print());
	...
	void me13() throws Exception {
		ResultActions resultActions = mvc.perform(
			get("/api/users/me").header("Authorization", "Bearer " + validRefreshToken)).andDo(print());
	...
	private ResultActions refreshAccessTokenHeaderRequest(String refreshToken, String token) throws Exception {
		return mvc.perform(post("/api/users/refresh").header("Authorization", "Bearer " + token)
			.content("""
				{
				  "refreshToken": "%s"
				}
	...
	void updateProfile1() throws Exception {
		...
		ResultActions resultActions = mvc.perform(
			put("/api/users/me").header("Authorization", "Bearer " + validToken).content("""
				{
				  "nickname": "%s",
				  "address": "%s",
				  "email": "%s"
				}
				""".formatted(newNickname, newAddress, newEmail)).contentType(MediaType.APPLICATION_JSON));
	...
	void updateProfile2() throws Exception {
		...
		ResultActions resultActions = mvc.perform(
			put("/api/users/me").header("Authorization", "Bearer " + validToken).content("""
				...
				""".formatted(duplicateEmail)).contentType(MediaType.APPLICATION_JSON));
	...
	void deleteProfile1() throws Exception {
		ResultActions resultActions = mvc.perform(
			delete("/api/users/me").header("Authorization", "Bearer " + validToken));
	...
	void deleteProfile3() throws Exception {
		ResultActions resultActions = mvc.perform(
			delete("/api/users/me").header("Authorization", "Bearer wrong-token"));
